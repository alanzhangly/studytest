...
//        GenerateImage(strImg, "D:\\wangyc.jpg");
        ...
        // 测试从图片文件转换为Base64编码
        System.out.println(GetImageStr("C:\\Users\\89\\Desktop\\2.jpg"));
    }
    ...
    public static byte[] GetImageBytes(String imgFilePath) {// 将图片文件转化为字节数组字符串，并对其进行Base64编码处理
        byte[] data = null;
        // 读取图片字节数组
        try {
            InputStream in = new FileInputStream(imgFilePath);
    ...
    public static byte[] getByteByUrl(String url){
        ...
        try{
            ...
            InputStream input = new BufferedInputStream(netUrl.openConnection().getInputStream());
            data = new byte[netUrl.openConnection().getContentLength()];
            input.read(data);
            input.close();
        }catch(Exception e){
            e.printStackTrace();
