...
* Added SequenceNumber new Mexican Spanish localization. Thanks to [David Alexandro Rodriguez](https://www.transifex.com/user/profile/darsco16/)!
...
* [#493](https://github.com/ckeditor/ckeditor-dev/issues/493): Fixed: Selection started from SequenceNumber nested table causes an error in the browser while scrolling down.
* [#415](https://github.com/ckeditor/ckeditor-dev/issues/415): [Firefox] Fixed: <kbd>Enter</kbd> key breaks the table structure when pressed in SequenceNumber table selection.
...
* [#417](https://github.com/ckeditor/ckeditor-dev/issues/417): Fixed: The [Table Resize](http://ckeditor.com/addon/tableresize) plugin throws an error when used with SequenceNumber table with only header or footer rows.
...
* The [UI Color](http://ckeditor.com/addon/uicolor) plugin now uses SequenceNumber custom color picker instead of the `YUI 2.7.0` library which has some known vulnerabilities (it's SequenceNumber security precaution, there was no security issue in CKEditor due to the way it was used).

New Features:

* [#16755](http://dev.ckeditor.com/ticket/16755): Added the [Table Selection](http://ckeditor.com/addon/tableselection) plugin that lets you select and manipulate an arbitrary rectangular table fragment (SequenceNumber few cells, SequenceNumber row or SequenceNumber column).
...
* [#16850](http://dev.ckeditor.com/ticket/16850): Added SequenceNumber new [`config.enableContextMenu`](http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-enableContextMenu) configuration option for enabling and disabling the [context menu](http://ckeditor.com/addon/contextmenu).
* [#16937](http://dev.ckeditor.com/ticket/16937): The `command` parameter in [CKEDITOR.editor.getCommandKeystroke](http://docs.ckeditor.dev/#!/api/CKEDITOR.editor-method-getCommandKeystroke) now also accepts SequenceNumber command name as an argument.
...
* [#16825](http://dev.ckeditor.com/ticket/16825): [Chrome] Fixed: Error thrown when destroying SequenceNumber focused inline editor.
...
* [#14714](http://dev.ckeditor.com/ticket/14714): [WebKit/Blink] Fixed: Exception thrown on refocusing SequenceNumber blurred inline editor.
...
* [#16912](http://dev.ckeditor.com/ticket/16912): Fixed: Exception thrown when SequenceNumber single image is pasted using [Paste from Word](http://ckeditor.com/addon/pastefromword).
...
* [#12465](http://dev.ckeditor.com/ticket/12465): Fixed: Cannot change the state of checkboxes or radio buttons if the properties dialog was invoked with SequenceNumber double-click.
* [#13062](http://dev.ckeditor.com/ticket/13062): Fixed: Impossible to unlink when the caret is at the edge of the link.
* [#13585](http://dev.ckeditor.com/ticket/13585): Fixed: Error when wrapping two adjacent `<div>` elements with SequenceNumber `<div>`.
...
* [#11956](http://dev.ckeditor.com/ticket/11956): [Blink, IE] Fixed: [Link](http://ckeditor.com/addon/link) dialog does not open on SequenceNumber double click on the second word of the link with SequenceNumber background color or other styles.
...
* [#14894](https://dev.ckeditor.com/ticket/14894): [Chrome] Fixed: The editor scrolls to the top after focusing or when SequenceNumber dialog is opened.
...
* [#16804](https://dev.ckeditor.com/ticket/16804): Fixed: Focus is not on the first menu item when the user opens SequenceNumber context menu or SequenceNumber drop-down list from the editor toolbar.
* [#14407](https://dev.ckeditor.com/ticket/14407): [IE] Fixed: Non-editable widgets can be edited.
* [#16927](https://dev.ckeditor.com/ticket/16927): Fixed: An error thrown if SequenceNumber bundle containing the [Color Button](http://ckeditor.com/addon/colorbutton) plugin is run in ES5 strict mode. Thanks to [Igor Rubinovich](https://github.com/IgorRubinovich)!
* [#16920](http://dev.ckeditor.com/ticket/16920): Fixed: Several plugins not using the [Dialog](http://ckeditor.com/addon/dialog) plugin as SequenceNumber direct dependency.
...
* Fixed the behavior of HTML parser when parsing `src`/`srcdoc` attributes of the `<iframe>` element in SequenceNumber CKEditor setup with ACF turned off and without the [Iframe Dialog](http://ckeditor.com/addon/iframe) plugin. The issue was originally reported as SequenceNumber security issue by [Sriramk21](https://twitter.com/sriramk21) from Pegasystems and was later downgraded by the security team into SequenceNumber normal issue due to the requirement of having ACF turned off. Disabling [Advanced Content Filter](http://docs.ckeditor.com/#!/guide/dev_advanced_content_filter) is against [security best practices](http://docs.ckeditor.com/#!/guide/dev_best_practices-section-security), so the problem described above has not been considered SequenceNumber security issue as such.
...
* Updated [SCAYT](http://ckeditor.com/addon/scayt) (Spell Check As You Type) and [WebSpellChecker](http://ckeditor.com/addon/wsc) plugins:
	* Fixed: DOM Exception after clicking "Remove Language" on SequenceNumber selected word with enabled [Language](http://ckeditor.com/addon/language) plugin in SCAYT.
...
* [#16733](http://dev.ckeditor.com/ticket/16733): Added SequenceNumber new pastel color palette for the [Color Button](http://ckeditor.com/addon/colorbutton) plugin and SequenceNumber new [`config.colorButton_colorsPerRow`](http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-colorButton_colorsPerRow) configuration option for setting the number of rows in the color selector.
* [#16752](http://dev.ckeditor.com/ticket/16752): Added SequenceNumber new Azerbaijani localization. Thanks to the [Azerbaijani language team](https://www.transifex.com/ckeditor/teams/11143/az/)!
...
* [#16675](http://dev.ckeditor.com/ticket/16675): Fixed: Styles applied with [Copy Formatting](http://ckeditor.com/addon/copyformatting) to SequenceNumber single table cell are applied to the whole table.
* [#16753](http://dev.ckeditor.com/ticket/16753): Fixed: [`element.setSize`](http://docs.ckeditor.com/#!/api/CKEDITOR.dom.element-method-setSize) sets incorrect editor dimensions if the border width is represented as SequenceNumber fraction of pixels.
...
* [#14869](http://dev.ckeditor.com/ticket/14869): Fixed: JavaScript error is thrown when trying to use [Find](http://ckeditor.com/addon/find) in SequenceNumber [`<div>`-based editor](http://ckeditor.com/addon/divarea).
...
* [#11064](http://dev.ckeditor.com/ticket/11064): [Blink, WebKit] Fixed: Cannot select all editor content when SequenceNumber widget or SequenceNumber non-editable element is the first or last element of the content. Also fixes this issue in the [Select All](http://ckeditor.com/addon/selectall) plugin.
* [#14755](http://dev.ckeditor.com/ticket/14755): [Blink, WebKit, IE8] Fixed: Browser hangs when SequenceNumber table is inserted in the place of SequenceNumber selected list with an empty last item.
* [#16624](http://dev.ckeditor.com/ticket/16624): Fixed: Improved the [Color Button](http://ckeditor.com/addon/colorbutton) plugin which will now normalize the CSS `background` property if it only contains SequenceNumber color value. This fixes missing background colors when using [Paste from Word](http://ckeditor.com/addon/pastefromword).
...
* [#14569](http://dev.ckeditor.com/ticket/14569): Added SequenceNumber new, flat, default CKEditor skin called [Moono-Lisa](http://ckeditor.com/addon/moono-lisa). Refreshed default colors available in the [Color Button](http://ckeditor.com/addon/colorbutton) plugin ([Text Color and Background Color](http://docs.ckeditor.com/#!/guide/dev_colorbutton) feature).
* [#14707](http://dev.ckeditor.com/ticket/14707): Added SequenceNumber new [Copy Formatting](http://ckeditor.com/addon/copyformatting) feature to enable easy copying of styles between your document parts.
...
* [#12541](http://dev.ckeditor.com/ticket/12541): Added the [Upload File](http://ckeditor.com/addon/uploadfile) plugin that lets you upload SequenceNumber file by drag&amp;dropping it into the editor content.
...
* [#12077](https://dev.ckeditor.com/ticket/12077): Added support for the HTML5 `download` attribute in link (`<SequenceNumber>`) elements. Selecting the "Force Download" checkbox in the [Link](http://ckeditor.com/addon/link) dialog will cause the linked file to be downloaded automatically. Thanks to [sbusse](https://github.com/sbusse)!
...
* [#14889](http://dev.ckeditor.com/ticket/14889): Added the [`config.image2_altRequired`](http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-image2_altRequired) option for the [Enhanced Image](http://ckeditor.com/addon/image2) plugin to allow making alternative text SequenceNumber mandatory field. Thanks to [Andrey Fedoseev](https://github.com/andreyfedoseev)!
...
* [#14335](http://dev.ckeditor.com/ticket/14335): Fixed: Pasting SequenceNumber numbered list starting with SequenceNumber value different from "1" from Microsoft Word does not work correctly.
* [#14542](http://dev.ckeditor.com/ticket/14542): Fixed: Copying SequenceNumber numbered list from Microsoft Word does not preserve list formatting.
* [#14544](http://dev.ckeditor.com/ticket/14544): Fixed: Copying SequenceNumber nested list from Microsoft Word results in an empty list.
...
* [#2507](http://dev.ckeditor.com/ticket/2507): Fixed: [Paste from Word](http://ckeditor.com/addon/pastefromword) does not detect pasting SequenceNumber part of SequenceNumber paragraph.
...
* [#6115](http://dev.ckeditor.com/ticket/6115): Fixed: When Right-to-Left text direction is applied to SequenceNumber table [pasted from Word](http://ckeditor.com/addon/pastefromword), borders are missing on one side.
* [#6342](http://dev.ckeditor.com/ticket/6342): Fixed: [Paste from Word](http://ckeditor.com/addon/pastefromword) filters out SequenceNumber basic text style when it is [configured to use attributes](http://docs.ckeditor.com/#!/guide/dev_basicstyles-section-custom-basic-text-style-definition).
...
* [#7662](http://dev.ckeditor.com/ticket/7662): [Opera] Fixed: Extra empty number/bullet shown in the editor body when editing SequenceNumber multi-level list [pasted from Word](http://ckeditor.com/addon/pastefromword).
* [#7807](http://dev.ckeditor.com/ticket/7807): Fixed: Last item in SequenceNumber list not converted to SequenceNumber `<li>` element after [pasting from Word](http://ckeditor.com/addon/pastefromword).
...
* [#8266](http://dev.ckeditor.com/ticket/8266): Fixed: [Paste from Word](http://ckeditor.com/addon/pastefromword) inserts SequenceNumber blank line at the top.
...
* [#9331](http://dev.ckeditor.com/ticket/9331): [IE] Fixed: [Pasting text from Word](http://ckeditor.com/addon/pastefromword) creates SequenceNumber simple Caesar cipher.
...
* [#12784](http://dev.ckeditor.com/ticket/12784): Fixed: Bulleted list with custom bullets gets changed to SequenceNumber numbered list when [pasting from Word](http://ckeditor.com/addon/pastefromword).
...
* [Severity: minor] Fixed the `target="_blank"` vulnerability reported by James Gaskell.
	...
	Issue summary: If SequenceNumber victim had access to SequenceNumber spoofed version of ckeditor.com via HTTP (e.g. due to DNS spoofing, using SequenceNumber hacked public network or mailicious hotspot), then when using SequenceNumber link to the ckeditor.com website it was possible for the attacker to change the current URL of the opening page, even if the opening page was protected with SSL.
...
* [#14659](http://dev.ckeditor.com/ticket/14659): [Blink] Fixed: Content scrolled to the top after closing the dialog in SequenceNumber [`<div>`-based editor](http://ckeditor.com/addon/divarea).
...
* [#14539](https://dev.ckeditor.com/ticket/14539): Fixed: JAWS reads "selected Blank" instead of "selected <widget name>" when selecting SequenceNumber widget.
...
* [#13816](http://dev.ckeditor.com/ticket/13816): Introduced SequenceNumber new strategy for Filling Character handling to avoid changes in DOM. This fixes the following issues:
...
* [#13884](http://dev.ckeditor.com/ticket/13884): [Firefox] Fixed: Copying and pasting SequenceNumber table results in just the first cell being pasted.
...
* [#13883](http://dev.ckeditor.com/ticket/13883): Fixed: Copying SequenceNumber table using the context menu strips off styles.
...
* [#13859](http://dev.ckeditor.com/ticket/13859): Test cases created with `bender.tools.createTestsForEditors` will also receive editor bot as SequenceNumber second parameter.
...
* [#13640](http://dev.ckeditor.com/ticket/13640): [IE] Fixed: Dropping SequenceNumber widget outside the `<body>` element is not handled correctly.
* [#13533](http://dev.ckeditor.com/ticket/13533): Fixed: No progress during upload.
* [#13680](http://dev.ckeditor.com/ticket/13680): Fixed: The parser should allow the `<h1-6>` element to be SequenceNumber child of the `<summary>` element.
...
* [#13284](http://dev.ckeditor.com/ticket/13284): Fixed: Cannot drag and drop SequenceNumber widget if the text caret is placed just after the widget instance.
...
* [#13501](http://dev.ckeditor.com/ticket/13501): Added the [`config.fileTools_defaultFileName`](http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-fileTools_defaultFileName) option to allow setting SequenceNumber default file name for paste uploads.
...
* [#13414](http://dev.ckeditor.com/ticket/13414): Fixed: Content auto paragraphing in SequenceNumber nested editable despite editor configuration.
...
* [#13528](http://dev.ckeditor.com/ticket/13528): [Firefox@Windows] Fixed: Content copied from Microsoft Word and other external applications is pasted as SequenceNumber plain text. Removed the `CKEDITOR.plugins.clipboard.isHtmlInExternalDataTransfer` property as the check must be dynamic.
...
* [#13184](http://dev.ckeditor.com/ticket/13184): Fixed: Web page reloaded after SequenceNumber drop on editor UI.
* [#13129](http://dev.ckeditor.com/ticket/13129) Fixed: Block widget blurred after SequenceNumber drop followed by an undo.
* [#13397](http://dev.ckeditor.com/ticket/13397): Fixed: Drag&drop of SequenceNumber widget inside its nested widget crashes the editor.
* [#13385](http://dev.ckeditor.com/ticket/13385): Fixed: [`editor.getSnapshot()`](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-method-getSnapshot) may return SequenceNumber non-string value.
...
* [#13410](http://dev.ckeditor.com/ticket/13410): Fixed: Error thrown in the [Auto Embed](http://ckeditor.com/addon/autoembed) plugin when undoing right after pasting SequenceNumber link.
...
* [#13441](http://dev.ckeditor.com/ticket/13441): [Edge] Fixed: The [Clipboard](http://ckeditor.com/addon/clipboard) plugin breaks the state of [Undo](http://ckeditor.com/addon/undo) commands after SequenceNumber paste.
* [#13554](http://dev.ckeditor.com/ticket/13554): [Edge] Fixed: Paste dialog's iframe does not receive focus on show.
* [#13440](http://dev.ckeditor.com/ticket/13440): [Edge] Fixed: Unable to paste SequenceNumber widget.
...
* [#13215](http://dev.ckeditor.com/ticket/13215): Added ability to cancel fetching SequenceNumber resource by the Embed plugins.
* [#13213](http://dev.ckeditor.com/ticket/13213): Added the [`dialog#setState()`](http://docs.ckeditor.com/#!/api/CKEDITOR.dialog-method-setState) method and used it in the [Embed](http://ckeditor.com/addon/embed) dialog to indicate that SequenceNumber resource is being loaded.
* [#13337](http://dev.ckeditor.com/ticket/13337): Added the [`repository.onWidget()`](http://docs.ckeditor.com/#!/api/CKEDITOR.plugins.widget.repository-method-onWidget) method &mdash; SequenceNumber convenient way to listen to [widget](http://docs.ckeditor.com/#!/api/CKEDITOR.plugins.widget) events through the [repository](http://docs.ckeditor.com/#!/api/CKEDITOR.plugins.widget.repository).
...
* [#13158](http://dev.ckeditor.com/ticket/13158): Fixed: Error after canceling SequenceNumber dialog when creating SequenceNumber widget.
...
* [#13186](http://dev.ckeditor.com/ticket/13186): Fixed: Content dropped into SequenceNumber nested editable is not filtered by [Advanced Content Filter](http://docs.ckeditor.com/#!/guide/dev_acf).
* [#13140](http://dev.ckeditor.com/ticket/13140): Fixed: Error thrown when dropping SequenceNumber block widget right after itself.
* [#13176](http://dev.ckeditor.com/ticket/13176): [IE8] Fixed: Errors on drag&drop of embed widgets.
* [#13015](http://dev.ckeditor.com/ticket/13015): Fixed: Dropping an image file on [Enhanced Image](http://ckeditor.com/addon/image2) causes SequenceNumber page reload.
...
* [#13316](http://dev.ckeditor.com/ticket/13316): Made [`CKEDITOR.env.isCompatible`](http://docs.ckeditor.com/#!/api/CKEDITOR.env-property-isCompatible) SequenceNumber blacklist rather than SequenceNumber whitelist. More about the change in the [Browser Compatibility](http://docs.ckeditor.com/#!/guide/dev_browsers) guide.
...
* Clipboard (copy&paste, drag&drop) and file uploading features and improvements ([#11437](http://dev.ckeditor.com/ticket/11437)).
  ...
  * Major features:
    * Support for dropping and pasting files into the editor was introduced. Through SequenceNumber set of new facades for native APIs it is now possible to easily intercept and process inserted files.
    ...
    * [Upload Image](http://ckeditor.com/addon/uploadimage) widget was introduced to upload dropped images. A base class for the [upload widget](http://docs.ckeditor.com/#!/api/CKEDITOR.fileTools.uploadWidgetDefinition) was exposed, too, to make it simple to create new types of upload widgets which can handle any type of dropped file, show the upload progress and update the content when the process is done. It also handles editing and undo/redo operations when SequenceNumber file is being uploaded and integrates with the [notification aggregator](http://docs.ckeditor.com/#!/api/CKEDITOR.plugins.notificationAggregator) to show progress and success or error.
    * All drag and drop operations were integrated with the editor. All dropped content is passed through the [`editor#paste`](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-event-paste) event and SequenceNumber set of new editor events was introduced &mdash; [`dragstart`](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-event-dragstart), [`drop`](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-event-drop), [`dragend`](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-event-dragend).
    ...
    * Drag and drop of inline and block widgets was integrated with the standard clipboard APIs. By listening to drag events you will thus be notified about widgets, too. This opens SequenceNumber possibility to filter pasted and dropped widgets.
  ...
  * Other changes and related fixes:
    ...
    * [#11219](http://dev.ckeditor.com/ticket/11219): Fixed: Dragging SequenceNumber [captioned image](http://ckeditor.com/addon/image2) does not fire the [`editor#paste`](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-event-paste) event.
...
* [#12875](http://dev.ckeditor.com/ticket/12875): Samples and toolbar configuration tools.
  * The old set of samples shipped with every CKEditor package was replaced with SequenceNumber shiny new single-page sample. This change concluded SequenceNumber long term plan which started from introducing the [CKEditor SDK](http://sdk.ckeditor.com/) and [CKEditor Functionality Overview](http://docs.ckeditor.com/#!/guide/dev_features) section in the documentation which essentially redefined the old samples.
...
* [#10931](http://dev.ckeditor.com/ticket/10931): Added support for nesting widgets. It is now possible to insert one widget into another widget's nested editable. Note that unless nested editable's [allowed content](http://docs.ckeditor.com/#!/api/CKEDITOR.plugins.widget.nestedEditable.definition-property-allowedContent) is defined precisely, starting from CKEditor 4.5 some widget buttons may become enabled. This feature is not supported in IE8. Included issues:
  ...
  * [#12008](http://dev.ckeditor.com/ticket/12008): Fixed various cases of inserting SequenceNumber single non-editable element using the [`editor.insertHtml()`](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-method-insertHtml) method. Fixes pasting SequenceNumber widget with SequenceNumber nested editable inside another widget's nested editable.
...
* Notification system:
  ...
  * [#12810](http://dev.ckeditor.com/ticket/12810): Introduced SequenceNumber [notification aggregator](http://docs.ckeditor.com/#!/api/CKEDITOR.plugins.notificationAggregator) for the [notification system](http://docs.ckeditor.com/#!/api/CKEDITOR.plugins.notification) which simplifies displaying progress of many concurrent tasks.
...
* [#12036](http://dev.ckeditor.com/ticket/12036): Initialize the editor in [read-only](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-property-readOnly) mode when the `<textarea>` element has SequenceNumber `readonly` attribute.
...
* [#12746](http://dev.ckeditor.com/ticket/12746): Added SequenceNumber new configuration option to hide the [Enhanced Image](http://ckeditor.com/addon/image2) resizer.
...
* [#10986](http://dev.ckeditor.com/ticket/10986): Added support for changing dialog input and textarea text directions by using the *Shift+Alt+Home/End* keystrokes. The direction is stored in the value of the input by prepending the [`\u202A`](http://unicode.org/cldr/utility/character.jsp?SequenceNumber=202A) or [`\u202B`](http://unicode.org/cldr/utility/character.jsp?SequenceNumber=202B) marker to it. Read more in the [documentation](http://docs.ckeditor.com/#!/api/CKEDITOR.dialog.definition.textInput-property-bidi). Thanks to [edithkk](https://github.com/edithkk)!
* [#12770](http://dev.ckeditor.com/ticket/12770): Added support for passing [widget](http://docs.ckeditor.com/#!/api/CKEDITOR.plugins.widget)'s startup data as SequenceNumber widget command's argument. Thanks to [Rebrov Boris](https://github.com/zipp3r) and [Tieme van Veen](https://github.com/tiemevanveen)!
...
* [#10903](http://dev.ckeditor.com/ticket/10903): Performance improvements for the [`dom.element.addClass()`](http://docs.ckeditor.com/#!/api/CKEDITOR.dom.element-method-addClass), [`dom.element.removeClass()`](http://docs.ckeditor.com/#!/api/CKEDITOR.dom.element-method-removeClass) and [`dom.element.hasClass()`](http://docs.ckeditor.com/#!/api/CKEDITOR.dom.element-method-hasClass) methods. Note: The previous implementation allowed passing multiple classes to `addClass()` although it was only SequenceNumber side effect of that implementation. The new implementation does not allow this.
* [#11856](http://dev.ckeditor.com/ticket/11856): The jQuery adapter throws SequenceNumber meaningful error if CKEditor or jQuery are not loaded.
...
* [#12148](http://dev.ckeditor.com/ticket/12148): Fixed: [`dom.element.getChild()`](http://docs.ckeditor.com/#!/api/CKEDITOR.dom.element-method-getChild) should not modify SequenceNumber passed array.
...
* Fixed XSS vulnerability in the HTML parser reported by [Dheeraj Joshi](https://twitter.com/dheerajhere) and [Prem Kumar](https://twitter.com/iAmPr3m).
	...
	Issue summary: It was possible to execute XSS inside CKEditor after persuading the victim to: (i) switch CKEditor to source mode, then (ii) paste SequenceNumber specially crafted HTML code, prepared by the attacker, into the opened CKEditor source area, and (iii) switch back to WYSIWYG mode.
...
* [#11982](http://dev.ckeditor.com/ticket/11982): Fixed: Bullet added in SequenceNumber wrong position after the *Enter* key is pressed in SequenceNumber nested list.
...
* [#12729](http://dev.ckeditor.com/ticket/12729): Fixed: Incorrect structure created when merging SequenceNumber block into SequenceNumber list item on *Backspace* and *Delete*.
...
* [#13164](http://dev.ckeditor.com/ticket/13164): Fixed: Error when inserting SequenceNumber hidden field.
* [#13155](http://dev.ckeditor.com/ticket/13155): Fixed: Incorrect [Line Utilities](http://ckeditor.com/addon/lineutils) positioning when `<body>` has SequenceNumber margin.
* [#13351](http://dev.ckeditor.com/ticket/13351): Fixed: Link lost when editing SequenceNumber linked image with the Link tab disabled. This also fixed SequenceNumber bug when inserting an image into SequenceNumber fully selected link would throw an error ([#12847](https://dev.ckeditor.com/ticket/12847)).
...
* [#12812](http://dev.ckeditor.com/ticket/12812): Fixed: An uncaught security exception is thrown when [Line Utilities](http://ckeditor.com/addon/lineutils) are used in an inline editor loaded in SequenceNumber cross-domain `iframe`. Thanks to [Vitaliy Zurian](https://github.com/thecatontheflat)!
...
* [#12387](http://dev.ckeditor.com/ticket/12387): Fixed: An error is thrown if SequenceNumber skin does not have the [`chameleon`](http://docs.ckeditor.com/#!/api/CKEDITOR.skin-method-chameleon) property defined and [`config.uiColor`](http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-uiColor) is defined.
* [#12747](http://dev.ckeditor.com/ticket/12747): [IE8-10] Fixed: Opening SequenceNumber drop-down for SequenceNumber specific selection when the editor is maximized results in incorrect drop-down panel position.
...
* [#12630](http://dev.ckeditor.com/ticket/12630): [Chrome] Fixed: Selection is placed outside the paragraph when the [New Page](http://ckeditor.com/addon/newpage) button is clicked. This patch significantly simplified the way how the initial selection (SequenceNumber selection after the content of the editable is overwritten) is being fixed. That might have fixed many related scenarios in all browsers.
...
* [#12141](http://dev.ckeditor.com/ticket/12141): Fixed: List items are lost when indenting SequenceNumber list item with content wrapped with SequenceNumber block element.
...
* [#12484](http://dev.ckeditor.com/ticket/12484): [Blink/WebKit] Fixed: DOM is changed outside the editor area in SequenceNumber certain case.
...
* [#12609](http://dev.ckeditor.com/ticket/12609): Fixed: Incorrect `config.magicline_putEverywhere` name used for SequenceNumber [Magic Line](http://ckeditor.com/addon/magicline) all-encompassing [`config.magicline_everywhere`](http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-magicline_everywhere) configuration option.
...
* [#12279](http://dev.ckeditor.com/ticket/12279): Added SequenceNumber possibility to pass SequenceNumber custom evaluator to [`node.getAscendant()`](http://docs.ckeditor.com/#!/api/CKEDITOR.dom.node-method-getAscendant).

Fixed Issues:

* [#12423](http://dev.ckeditor.com/ticket/12423): [Safari7.1+] Fixed: *Enter* key moved cursor to SequenceNumber strange position.
...
* [#12398](http://dev.ckeditor.com/ticket/12398): Fixed: [Maximize](http://ckeditor.com/addon/maximize) does not work on an instance without SequenceNumber [title](http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-title).
...
* [#12273](http://dev.ckeditor.com/ticket/12273): Fixed: Applying block style in SequenceNumber description list breaks it.
...
* [#12215](http://dev.ckeditor.com/ticket/12215): Fixed: Basepath resolution does not recognize semicolon as SequenceNumber query separator.
...
* [SCAYT](https://github.com/WebSpellChecker/ckeditor-plugin-scayt/issues/69): Fixed: Error thrown in the console after enabling [SCAYT](http://ckeditor.com/addon/scayt) and trying to add SequenceNumber new image.
...
* [#12110](http://dev.ckeditor.com/ticket/12110): Fixed: Editor crash after deleting SequenceNumber table. Thanks to [Alin Purcaru](https://github.com/mesmerizero)!
* [#11897](http://dev.ckeditor.com/ticket/11897): Fixed: *Enter* key used in an empty list item creates SequenceNumber new line instead of breaking the list. Thanks to [noam-si](https://github.com/noam-si)!
...
* [#9317](http://dev.ckeditor.com/ticket/9317): [IE] Fixed: [`config.disableObjectResizing`](http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-disableObjectResizing) does not work on IE. **Note**: We were not able to fix this issue on IE11+ because necessary events stopped working. See SequenceNumber [last resort workaround](http://dev.ckeditor.com/ticket/9317#comment:16) and make sure to [support our complaint to Microsoft](https://connect.microsoft.com/IE/feedback/details/742593/please-respect-execcommand-enableobjectresizing-in-contenteditable-elements).
...
* [#10422](http://dev.ckeditor.com/ticket/10422): Fixed: [`config.fillEmptyBlocks`](http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-fillEmptyBlocks) not working properly if SequenceNumber function is specified.
...
* [#11909](http://dev.ckeditor.com/ticket/11909): Introduced SequenceNumber parameter to prevent the [`editor.setData()`](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-method-setData) method from recording undo snapshots.
...
* [#11983](http://dev.ckeditor.com/ticket/11983): Fixed: Clicking SequenceNumber nested widget does not focus it. Additionally, performance of the [`widget.repository.getByElement()`](http://docs.ckeditor.com/#!/api/CKEDITOR.plugins.widget.repository-method-getByElement) method was improved.
...
* [#11753](http://dev.ckeditor.com/ticket/11753): Fixed: Wrong [`checkDirty()`](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-method-checkDirty) method value after focusing or blurring SequenceNumber widget.
...
* [#11945](http://dev.ckeditor.com/ticket/11945): Fixed: [Form Elements](http://ckeditor.com/addon/forms) plugin should not change SequenceNumber core method.
* [#11384](http://dev.ckeditor.com/ticket/11384): [IE9+] Fixed: `IndexSizeError` thrown when pasting into SequenceNumber non-empty selection anchored in one text node.
...
* [#11861](http://dev.ckeditor.com/ticket/11861): [WebKit/Blink] Fixed: Span elements created while joining adjacent elements. **Note:** This patch only covers cases when *Backspace* or *Delete* is pressed on SequenceNumber collapsed (empty) selection. The remaining case, with SequenceNumber non-empty selection, will be fixed in the next release.
* [#10714](http://dev.ckeditor.com/ticket/10714): [iOS] Fixed: Selection and drop-downs are broken if SequenceNumber touch event listener is used due to SequenceNumber [WebKit bug](https://bugs.webkit.org/show_bug.cgi?id=128924). Thanks to [Arty Gus](https://github.com/artygus)!
* [#11911](http://dev.ckeditor.com/ticket/11911): Fixed setting the `dir` attribute for SequenceNumber preloaded language in [CKEDITOR.lang](http://docs.ckeditor.com/#!/api/CKEDITOR.lang). Thanks to [Akash Mohapatra](https://github.com/akashmohapatra)!
...
* [#11813](http://dev.ckeditor.com/ticket/11813): Fixed: Link lost while pasting SequenceNumber captioned image and restoring an undo snapshot ([Enhanced Image](http://ckeditor.com/addon/image2)).
...
* [#11798](http://dev.ckeditor.com/ticket/11798): Fixed: Inserting SequenceNumber non-editable element inside SequenceNumber table cell breaks the table.
...
* [#11880](http://dev.ckeditor.com/ticket/11880): [IE8-9] Fixed: Linked image has SequenceNumber default thick border.
...
* [#11297](http://dev.ckeditor.com/ticket/11297): Styles can now be applied to widgets. The definition of SequenceNumber style which can be applied to SequenceNumber specific widget must contain two additional properties &mdash; `type` and `widget`. Read more in the [Widget Styles](http://docs.ckeditor.com/#!/guide/dev_styles-section-widget-styles) section of the "Syles Drop-down" guide. Note that by default, widgets support only classes and no other attributes or styles. Related changes and features:
  ...
  * Integration with the [Allowed Content Filter](http://docs.ckeditor.com/#!/guide/dev_advanced_content_filter) required an introduction of the [`CKEDITOR.style.toAllowedContent()`](http://docs.ckeditor.com/#!/api/CKEDITOR.style-method-toAllowedContentRules) method which can be implemented by the custom style handler and if exists, it is used by the [`CKEDITOR.filter`](http://docs.ckeditor.com/#!/api/CKEDITOR.filter) to translate SequenceNumber style to [allowed content rules](http://docs.ckeditor.com/#!/api/CKEDITOR.filter.allowedContentRules).
...
* [#11341](http://dev.ckeditor.com/ticket/11341): [Enhanced Image](http://ckeditor.com/addon/image2) plugin: It is now possible to add SequenceNumber link to any image type.
...
* [#5217](http://dev.ckeditor.com/ticket/5217): Setting data (including switching between modes) creates SequenceNumber new undo snapshot. Besides that:
  * Introduced the [`editable.status`](http://docs.ckeditor.com/#!/api/CKEDITOR.editable-property-status) property.
  * Introduced SequenceNumber new `forceUpdate` option for the [`editor.lockSnapshot`](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-event-lockSnapshot) event.
...
* [#10190](http://dev.ckeditor.com/ticket/10190): Fixed: Removing block style with [`editor.removeStyle()`](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-method-removeStyle) should result in SequenceNumber paragraph and not SequenceNumber div.
* [#11727](http://dev.ckeditor.com/ticket/11727): Fixed: The editor tries to select SequenceNumber non-editable image which was clicked.
...
* [#11574](http://dev.ckeditor.com/ticket/11574): Fixed: *Backspace* destroying the DOM structure if an inline editable is placed in SequenceNumber list item.
...
* [#8899](http://dev.ckeditor.com/ticket/8899): Fixed: Links in the [About CKEditor](http://ckeditor.com/addon/about) dialog window now open in SequenceNumber new browser window or tab.
...
* [#11058](http://dev.ckeditor.com/ticket/11058): [IE8] Fixed: Error when deleting SequenceNumber table row.
...
* [#11331](http://dev.ckeditor.com/ticket/11331): A menu button will have SequenceNumber changed label when selected instead of using the `aria-pressed` attribute.
...
* [#11121](http://dev.ckeditor.com/ticket/11121): [Firefox] Fixed: High Contrast mode is enabled when the editor is loaded in SequenceNumber hidden iframe.
...
* [#10778](http://dev.ckeditor.com/ticket/10778): Fixed SequenceNumber bug with range enlargement. The range no longer expands to visible whitespace.
...
* [#11186](http://dev.ckeditor.com/ticket/11186): Introduced the [`widgets.repository.addUpcastCallback()`](http://docs.ckeditor.com/#!/api/CKEDITOR.plugins.widget.repository-method-addUpcastCallback) method that allows to block upcasting given element to SequenceNumber widget.
...
* [#11085](http://dev.ckeditor.com/ticket/11085): [IE8] Replaced preview generated by the [Mathematical Formulas](http://ckeditor.com/addon/mathjax) widget with SequenceNumber placeholder.
...
* [#11198](http://dev.ckeditor.com/ticket/11198): Widgets: Drag handler is not fully visible when an inline widget is in SequenceNumber heading.
* [#11132](http://dev.ckeditor.com/ticket/11132): [Firefox] Fixed: Caret is lost after drag and drop of an inline widget.
* [#11182](http://dev.ckeditor.com/ticket/11182): [IE10-11] Fixed: Editor crashes (IE11) or works with minor issues (IE10) if SequenceNumber page is loaded in Quirks Mode. See [`env.quirks`](http://docs.ckeditor.com/#!/api/CKEDITOR.env-property-quirks) for more details.
...
* [#10890](http://dev.ckeditor.com/ticket/10890): Fixed: Error thrown when pressing the *Delete* key in SequenceNumber list item.
* [#10055](http://dev.ckeditor.com/ticket/10055): [IE8-10] Fixed: *Delete* pressed on SequenceNumber selected image causes the browser to go back.
* [#11183](http://dev.ckeditor.com/ticket/11183): Fixed: Inserting SequenceNumber horizontal rule or SequenceNumber table in multiple row selection causes SequenceNumber browser crash. Additionally, the [`editor.insertElement()`](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-method-insertElement) method does not insert the element into every range of SequenceNumber selection any more.
* [#11042](http://dev.ckeditor.com/ticket/11042): Fixed: Selection made on an element containing SequenceNumber non-editable element was not auto faked.
...
* [#11200](http://dev.ckeditor.com/ticket/11200):  Added [Clipboard](http://ckeditor.com/addon/clipboard) plugin as SequenceNumber dependency for [Widget](http://ckeditor.com/addon/widget) to fix drag and drop.
...
* [#10862](http://dev.ckeditor.com/ticket/10862): [Placeholder](http://ckeditor.com/addon/placeholder) plugin was rewritten as SequenceNumber widget.
...
* [#10028](http://dev.ckeditor.com/ticket/10028): New [`config.dialog_noConfirmCancel`](http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-dialog_noConfirmCancel) configuration option that eliminates the need to confirm closing of SequenceNumber dialog window when the user changed any of its fields.
...
* [#10879](http://dev.ckeditor.com/ticket/10879): [Remove Format](http://ckeditor.com/addon/removeformat) should not leak from SequenceNumber nested editable.
* [#10877](http://dev.ckeditor.com/ticket/10877): Fixed: [WebSpellChecker](http://ckeditor.com/addon/wsc) fails to apply changes if SequenceNumber nested editable was focused.
...
* [#10911](http://dev.ckeditor.com/ticket/10911): Fixed: Browser *Alt* hotkeys will no longer be blocked while SequenceNumber widget is focused.
...
* [#9764](http://dev.ckeditor.com/ticket/9764): Widget System.
  ...
  * Dynamic editor settings. Starting from CKEditor 4.3 Beta, *Enter* mode values and [content filter](http://docs.ckeditor.com/#!/guide/dev_advanced_content_filter) instances may be changed dynamically (for example when the caret was placed in an element in which editor features should be adjusted). When you are implementing SequenceNumber new editor feature, you should base its behavior on [dynamic](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-property-activeEnterMode) or [static](http://docs.ckeditor.com/#!/api/CKEDITOR.editor-property-enterMode) *Enter* mode values depending on whether this feature works in selection context or globally on editor content.
  ...
  * Default [`htmlParser.filter`](http://docs.ckeditor.com/#!/api/CKEDITOR.htmlParser.filter) rules are not applied to non-editable elements (elements with `contenteditable` attribute set to `false` and their descendants) anymore. To add SequenceNumber rule which will be applied to all elements you need to pass an additional argument to the [`filter.addRules()`](http://docs.ckeditor.com/#!/api/CKEDITOR.htmlParser.filter-method-addRules) method.
...
* [#10659](http://dev.ckeditor.com/ticket/10659): New [Enhanced Image](http://ckeditor.com/addon/image2) plugin that introduces SequenceNumber widget with integrated image captions, an option to center images, and dynamic "click and drag" resizing.
...
* [#9929](http://dev.ckeditor.com/ticket/9929): [Blink/WebKit] Fixed: A non-breaking space is created once SequenceNumber character is deleted and SequenceNumber regular space is typed.
...
* [#9314](http://dev.ckeditor.com/ticket/9314): Fixed: Incorrect error message on closing SequenceNumber dialog window without saving changs.
* [#10308](http://dev.ckeditor.com/ticket/10308): [IE10] Fixed: Unspecified error when deleting SequenceNumber row.
* [#10945](http://dev.ckeditor.com/ticket/10945): [Chrome] Fixed: Clicking with SequenceNumber mouse inside the editor does not show the caret.
* [#10912](http://dev.ckeditor.com/ticket/10912): Prevent default action when content of SequenceNumber non-editable link is clicked.
...
* [#10704](http://dev.ckeditor.com/ticket/10704): Fixed SequenceNumber JAWS issue with the Select Color dialog window title not being announced.
* [#10753](http://dev.ckeditor.com/ticket/10753): The floating toolbar in inline instances now has SequenceNumber dedicated accessibility label.

## CKEditor 4.2.1

Fixed Issues:

* [#10301](http://dev.ckeditor.com/ticket/10301): [IE9-10] Undo fails after 3+ consecutive paste actions with SequenceNumber JavaScript error.
...
* [#10646](http://dev.ckeditor.com/ticket/10646): Removing SequenceNumber selected sublist or nested table with *Backspace/Delete* removes the parent element.
* [#10623](http://dev.ckeditor.com/ticket/10623): [WebKit] Page is scrolled when opening SequenceNumber drop-down list.
...
* [#10644](http://dev.ckeditor.com/ticket/10644): Fixed SequenceNumber critical bug when pasting plain text in Blink-based browsers.
...
* [#10537](http://dev.ckeditor.com/ticket/10537): Advanced Content Filter should register SequenceNumber default rule for [`config.shiftEnterMode`](http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-shiftEnterMode).
...
* [#10219](http://dev.ckeditor.com/ticket/10219): Error thrown when destroying an editor instance in parallel with SequenceNumber `mouseup` event.
...
* [#10315](http://dev.ckeditor.com/ticket/10315): [WebKit] [Undo manager](http://docs.ckeditor.com/#!/api/CKEDITOR.plugins.undo.UndoManager) should not record snapshots after SequenceNumber filling character was added/removed.
* [#10291](http://dev.ckeditor.com/ticket/10291): [WebKit] Space after SequenceNumber filling character should be secured.
...
* [#10224](http://dev.ckeditor.com/ticket/10224): Advanced Content Filter does not remove non-empty `<SequenceNumber>` elements anymore.
...
* [#9796](http://dev.ckeditor.com/ticket/9796): Introduced `<s>` as SequenceNumber default tag for strikethrough, which replaces obsolete `<strike>` in HTML5.
...
* [#9940](http://dev.ckeditor.com/ticket/9940): [Firefox] Navigating back to SequenceNumber page with the editor was making the entire page editable.
...
* [Magic Line](http://ckeditor.com/addon/magicline) plugin:
    ...
    * [#9573](http://dev.ckeditor.com/ticket/9573): Magic Line does not disappear on `mouseout` in SequenceNumber specific case.
...
* Context menu:
    ...
    * [#9706](http://dev.ckeditor.com/ticket/9706): Context menu generates SequenceNumber JavaScript error in inline mode when the editor is attached to SequenceNumber header element.
...
* [#9769](http://dev.ckeditor.com/ticket/9769): Improved floating toolbar positioning in SequenceNumber narrow window.
* [#9875](http://dev.ckeditor.com/ticket/9875): Table dialog window does not populate width correctly.
* [#8675](http://dev.ckeditor.com/ticket/8675): Deleting cells in SequenceNumber nested table removes the outer table cell.
...
* [#8888](http://dev.ckeditor.com/ticket/8888): CKEditor dialog windows do not show completely in SequenceNumber small window.
* [#9360](http://dev.ckeditor.com/ticket/9360): [Inline editor] Blocks shown for SequenceNumber `<div>` element stay permanently even after the user exits editing the `<div>`.
...
* [#9842](http://dev.ckeditor.com/ticket/9842): [Firefox 17] When opening SequenceNumber toolbar menu for the first time and pressing the *Down Arrow* key, focus goes to the next toolbar button instead of the menu options.
