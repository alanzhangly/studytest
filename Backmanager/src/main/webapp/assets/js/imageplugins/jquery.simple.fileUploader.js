...
(function($) {
	...
	$.fn.fileUploader = function(config){
		...
		config = $.extend({}, {
			autoUpload: false,
			limit: false,
			buttonUpload: "button[name='px-submit']",
		...
		var px = {
			...
			init: function(_obj){
				px.form = $(_obj).parents('form');
				//prepend wrapper markup
				px.form.before(wrapper);
				
				//Wrap input button
				$(_obj).wrap('<div class="px-input-button" />');
				//px.form.children('.px-input-button').prepend('<span>'+ config.selectFileLabel +'</span>');
				var addBtn = jQuery('<span><a href="javascript:void(0);" class="pull-right" type="addImagePlugins"><img border="0" src="'+ctx+'/image/add.png" style="width: 16px; height: 16px; margin: 2px; cursor: pointer; float: right;" title="添加/修改图片"></a></span>');
				px.form.children('.px-input-button').prepend(addBtn);
//				px.form.children('.px-input-button').prepend('<span><a href="javascript:void(0);" id="tempHref" class="btn pull-right green"><i class="icon-plus"></i>'+ config.selectFileLabel +'</a>'+
//	                '</a></span>');
		...
		*/
		function html5Upload($form) {
			...
			if (file) {
				...
				$form.find(':input').each(function() {
					if (this.type != 'file') {
						if ($(this).attr('name')!="current_path") {
							fd.append($(this).attr('name'), $(this).val());
						}
					}
				});
				fd.append("current_path", $("#currentPath").val());
				...
				.complete(function(jqXHR, textStatus) {
					if (jqXHR.status=="200") {
//						var imgNames = [];
//						$("#showContent").find("span").each(function(index,element){
//							imgNames[index] = $(element).html();
//						});
						var data = jqXHR.responseText;
						if (data) {
							var array = eval('(' + data + ')');
							for (var i=0;i<array.length;i++) {
								//ImagePlugins.addSimpleItem({"place":$form.attr("_place"), "mutiple":$form.attr("_mutiple")});
								//ImagePlugins.addImgBySrc($form.attr("_place"), array[i].imgPath);
							}
						}
//								var item = array[i];
//								if(imgNames.length==0 || imgNames.toString().indexOf(item.fileName) == -1) {
//									ImageUpload.appendNewImg(item.filePath,item.fileSize,item.fileName,item.imgPath,"checked");
//								}
//							}
//						}
					}
